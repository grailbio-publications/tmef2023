...
```{r boxplot_by_stage_by_label, fig.width=8, fig.height=5}
...
generate_survival_odds_ratio_df <- function(surv_model) {
  ...
  surv_model_summary$coefficients |>
    ...
    dplyr::rename(odds_ratio = `exp(coef).y`,
                  odds_ratio_lcb = `lower .95`,
                  odds_ratio_ucb = `upper .95`,
                  pvalue = `Pr(>|z|)`) |>
    dplyr::select(variable, index, odds_ratio, odds_ratio_lcb, odds_ratio_ucb,
                  pvalue) |>
...
odds_ratio_plt <- function(survival_odds_ratio_df, title = "") {
  survival_odds_ratio_df |>
    ggplot2::ggplot(ggplot2::aes(y = index, x = odds_ratio)) +
      ggplot2::geom_point(shape = 18, size = 5) +
      ggplot2::geom_errorbarh(ggplot2::aes(xmin = odds_ratio_lcb,
                                           xmax = odds_ratio_ucb),
                              height = 0.25) +
      ggplot2::geom_vline(xintercept = 1, color = "red", linetype = "dashed",
                          cex = 1, alpha = 0.5) +
      ggplot2::scale_y_continuous(name = "", breaks=1:length(survival_odds_ratio_df$variable),
                                  labels = survival_odds_ratio_df$variable, trans = "reverse") +
...
surv_table_base_plt <- function(survival_odds_ratio_df) {
  survival_odds_ratio_df |>
    ggplot2::ggplot(ggplot2::aes(y = variable)) +
    ggplot2::ylab(NULL) +
    ggplot2::xlab("") +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5, size=12),
                   axis.text.x = ggplot2::element_text(color="white", hjust = -3, size = 25),
...
odds_ratio_tbl_plt <- function(survival_odds_ratio_df) {
  survival_odds_ratio_df |>
    surv_table_base_plt() +
    ...
    ggplot2::geom_text(ggplot2::aes(
      y = rev(index),
      x = 1,
      label = sprintf("%0.1f", round(odds_ratio, digits = 1))), size = 4) +
    ggplot2::ggtitle("OR")
}

odds_ratio_ci_tbl_plt <- function(survival_odds_ratio_df) {
  survival_odds_ratio_df |>
    surv_table_base_plt() +
    ggplot2::geom_text(ggplot2::aes(
      y = rev(index),
      ...
      label = sprintf("%0.1f - %0.1f",
                      round(odds_ratio_lcb, digits = 1),
                      round(odds_ratio_ucb, digits = 1))),
      size = 4) +
    ggplot2::ggtitle("95% CI")
}

forest_plot <- function(surv_model, return_grob = TRUE, title = "") {
  surv_model_odds_ratios <- surv_model |>
    generate_survival_odds_ratio_df()

  this_grob <- gridExtra::arrangeGrob(
    odds_ratio_plt(surv_model_odds_ratios, title = title),
    odds_ratio_tbl_plt(surv_model_odds_ratios),
    odds_ratio_ci_tbl_plt(surv_model_odds_ratios),
